...
> 为了让操作变得简单，我们假设当前需要旋转的节点是 x, 父亲是 y,祖父是 z

> $\text{操作}\ zig_{y} - zig_{x}$ ~~连续转两次儿子同理~~

> 同上所述 $\Delta \Phi(S) = \phi(x') + \phi(y') + \phi(z') - \phi(x) - \phi(y) - \phi(z)$

> 同样，因为 $\phi(x') = \phi(z)$， 所以 $\Delta \Phi(S) =  \phi(y') + \phi(z') - \phi(x) - \phi(y)$

> 同样是替换，因为 $\phi(x') \ge \phi(y') $, $\phi(x) \le \phi(y)$

> 所以 $\Delta \Phi(S) = \phi(x') + \phi(z') - \phi(x) - \phi(x)$

> $ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \,=  \phi(x') + \phi(z') - 2\phi(x)$

> 然后我们设 $|x| = a,\ |z'| = b$

> 所以 $\text{原式} = log\,a + log\,b - 2\,log\,|x|$

> $$ = log \frac{a \times b}{{|x|} ^ 2}$$

> $$ \text{因为}\ |x| \ge a + b\  \ \text{所以原式} \le log \frac{a \times b}{(a + b) ^ 2} \le log \frac{a \times b}{ 2ab} \le -1$$

> 所以我们设原式为 $f(x)$ ，所以 $-1 - f(x) \ge 0$

> 因为是非负数，我们可以通过加上 $-1 - f(x)$ 来化简式子

> 所以得到 $\Delta \Phi (S) = \phi(x') - \phi(x)$ ，时间复杂度为 $O(\phi(x') - \phi(x))$
...
> 除了最后一次旋转的代价之外，我们的复杂度为 $O(1 + \sum_{i = 1} ^ n \phi(x) - \phi(x_i)) = O(1 + \phi(x_n) - \phi(x_0))$

> 此时 $x_n$ 是整棵树的根， 所以 $\phi(x_n) = log\, |T|$
